...
* **Configuration**: NapCat address, LLM model API, TTS service address, character persona and emotion mapping, and the switches and parameters of every extended feature; the configuration can be saved as presets and switched at any time.
...
* **Chat History**: browse and manage all sessions, with batch deletion, import and export; a session file keeps the complete chat log instead of only the most recent messages; the list can be sorted by character (same character grouped, both across and within groups newest first), by newest conversation, or by oldest conversation — the last two sort globally by time without grouping by character.
